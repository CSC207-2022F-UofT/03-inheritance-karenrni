...
public abstract class Bag {
    ...
    public boolean addItem(String item) {
        if (getNumberOfContents() < getCapacity()) {
            String[] arr = new String[getNumberOfContents() + 1];
            arr[getNumberOfContents()] = item;
            if (getNumberOfContents() - 1 >= 0) System.arraycopy(this.contents, 0, arr, 0,
                    getNumberOfContents() - 1);
            /* for(int i = 0; i < getNumberOfContents() - 1; i++){
    ...
    public String popItem(){
        if(getNumberOfContents() == 0){
            ...
            this.numberOfContents -= 1;
            String[] arr = new String[getNumberOfContents()];
            String last = this.contents[getNumberOfContents()];
            if (getNumberOfContents() >= 0) System.arraycopy(this.contents, 0, arr, 0,
                    getNumberOfContents());
            return last;
        }
    }
